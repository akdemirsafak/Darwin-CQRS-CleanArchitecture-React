...
public class PlanService : IPlanService
{
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("api/plan", createPlanDto);
    ...
    {
        var response= await _httpClient.DeleteAsync($"api/plan/{id}");
        if (!response.IsSuccessStatusCode)
    ...
    {
        var response = await _httpClient.GetFromJsonAsync<DarwinResponse<List<PlanViewModel>>>("api/plan");
    ...
    {
        var response = await _httpClient.GetFromJsonAsync<DarwinResponse<PlanViewModel>>($"api/plan/{id}");
        if (response.Errors is not null)
            throw new Exception(response.Errors.ToString());
        return response.Data;
    }
    ...
    {
        var response = await _httpClient.PutAsJsonAsync($"api/plan/{updatePlanDto.Id}", updatePlanDto);
        if (!response.IsSuccessStatusCode)
